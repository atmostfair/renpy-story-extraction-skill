...
这个仓库不仅是一次性文档，也是一份会随着真实项目经验继续生长的 Codex skill。每次完成剧情提取任务后，Codex 都应把通用的新经验、踩坑点和用户偏好沉淀回本地安装的 `SKILL.md` 或直接引用的参考文件中，随后验证 skill 包、生成简洁的提交信息，并推送到配置好的 GitHub 远端。

同步方向以“本地学到的规则进入仓库”为主：先把本地 skill 中的新指导镜像到当前仓库 checkout，再提交并推送到 `origin`。提交前应先 fetch 或 pull 远端默认分支。如果 fork 中已经有远端改动，Codex 应尽量保留双方有效内容，去掉重复表述，优先保留本次任务中新学到的规则，并在验证后推送合并结果。

fork 不是只读副本。你 fork 这个仓库之后，skill 可以在你的使用过程中逐渐吸收你的项目类型、提取习惯、命名规则、审计标准和偏好，形成与你自己的 Ren'Py 工作流相匹配的分支，而不必完全沿用上游维护者的默认习惯。
