...
For secondary merged deliverables, prefer range-prefixed filenames such as `0001-0110_main_quests.txt` when the grouped files are meant to be read in order. If a manifest has explicit primary sections followed by fallback visible labels, keep the primary table-defined sections as contiguous ordered groups, then merge fallback labels by source module or another documented relationship so filename sorting still preserves the manifest order.

When a user asks to move short supplemental outputs back into story context, do not simply append every snippet to the end of a chapter. Use source-level anchors such as `jump`, `call`, `call screen`, `Jump(...)`, `Start(...)`, or the nearest surrounding extracted dialogue to insert each support label at the point where it becomes reachable. For open-map, shop, and repeatable-screen text with no single canonical click order, place it at earliest proven availability and document that policy instead of inventing a mandatory linear route.
...
1. Locate the Ren'Py script root.
   ...
   - If a requested folder is actually an NW.js/HTML package (for example it contains `Game.exe`, `package.json`, `www/`, Chromium DLLs, and `.pak` runtime files) and has no `.rpy`, `.rpyc`, `.rpa`, `game/`, or `assets/x-game/` Ren'Py payload, record it as non-Ren'Py and do not fabricate Ren'Py `storyxx_*.txt` outputs from unrelated web assets.
...
4. Determine game order.
   ...
   - Desktop builds may expose a chapter selector whose buttons jump first into intermediate questionnaire/setup labels, such as `qv*`, before those labels jump to the real chapter starts. Treat the selector plus the intermediate jump chain as ordering evidence, and use the final story-entry labels for split outputs rather than extracting UI/setup labels as standalone story chapters.
...
6. Map rendered names.
   ...
   - When scoped speaker overrides are used, document those scopes in the final speaker map or manifest. A clean story body is not enough if `speaker_map.txt` still implies one section-wide meaning for a reused key.
...
8. Audit and iterate.
   ...
   - When split outputs contain very short files, do not judge completeness by output line counts alone. Cross-check each short file against its exact source label/range, then run a resource-filtered, speaker-key-aware audit over unmanifested `.rpy` files to catch omitted supplemental event files without promoting image paths, UI style strings, or button assets as story text.
